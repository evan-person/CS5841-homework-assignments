...
# ╔═╡ 3ab26012-1050-4aec-9182-6c55349334dc
size(test_x)

# ╔═╡ 20a17676-5c2f-4dde-a838-928d3d057c1e
# ╠═╡ disabled = true
#=╠═╡
begin 
	flatten_train_x = reshape(train_x, 784, :)
    flatten_test_x = reshape(test_x, 784, :)
	dis = euclidean(flatten_train_x[:,1],flatten_test_x[:,1])
	print(typeof(flatten_train_x[:,1]))
...
function one_nearest_neighbor_classifier(train_x, train_y, test_x)
	...
	infer_labels = []
	for i in 1:size(flatten_test_x,2)
		min_distance = 1000000
		nearest_index = 0 

		for j in 1:size(flatten_train_x,2)
			distance = euclidean(flatten_train_x[:, j],flatten_test_x[:, i])
		...
		push!(infer_labels,train_y[nearest_index])
...
function accuracy(infer_labels, true_labels)
	...
	try
        tp = 0
        for i in eachindex(infer_labels)
            if Int(infer_labels[i]) == Int(true_labels[i])
                tp = tp + 1
            end
        end
        accuracy = tp/length(true_labels) 
        return accuracy
    catch e
...
# ╔═╡ 34e29c71-e179-49a3-b693-5dd07a3e92ce
# ╠═╡ disabled = true
#=╠═╡
begin
	infer_labels=one_nearest_neighbor_classifier(train_x, train_y, test_x)
	acc_knn=accuracy(infer_labels, test_y)
	print(acc_knn)
...
begin
	...
	k = 1:20
	acc = knn_leave_one_out(train_x,train_y, k)
	
	# acc = accuracy(predicted, test_y) #replace this with your real accuracy!!
...
function downsample_image(image, n)
	...
	#dowmsampled_image = image[1:n:end]
	for 
...
# ╠═╡ disabled = true
#=╠═╡
function knn_leave_one_out_experiment(train_x, train_y, test_x, test_y, k_values)
...
# ╔═╡ 0d62ee1b-e5d8-4bc2-aedc-afbe1ae29f1b
...
matrix, accuracy for each digit, and overall accuracy. Plot the overall test accuracy versus the regularization value where a log-scale is used for regularization value.
...
# ╔═╡ 569e1609-cd97-4312-a645-1ab4f7726600
...
# ╔═╡ 3af5f6ac-cc1a-4469-9d91-81fda755741a


# ╔═╡ 52e7f607-dabc-49fd-8aa5-6ebfccfc7ede
# ╠═╡ disabled = true
#=╠═╡
begin
	function mode(arr)
...
# 	# Randomly sample 2500 indices
# 	sample_indices = randperm(size(train_x, 3))[1:sample_size]
# 	println(size(train_y))
# 	println(size(train_x))
# 	# Select the corresponding samples from train_x and train_y
# 	sample_train_x = train_x[:, :, sample_indices]
# 	sample_train_y = train_y[sample_indices]
# 	println(size(sample_train_y))
# 	# Reshape the sampled data for KNN
# 	flatten_sample_train_x = reshape(sample_train_x, (784, sample_size))
# 	flatten_sample_train_y = reshape(sample_train_y, (1, sample_size))
	flatten_train_x = reshape(train_x[:, :, 1:10], 784, :)
	flatten_y = reshape(train_y, 1, :)
	flatten_train_y = flatten_y[:, 1:10]
	split_index = zeros(Int, size(flatten_train_x, 2))'
	split_index[end] = 1
	#shuffle!(split_index)
	split_index_1d = vec(split_index)
	LOOCV_train_x = flatten_train_x[:, split_index_1d .== 0]
	LOOCV_test_x = flatten_train_x[:, split_index_1d .== 1]
	LOOCV_train_y = flatten_train_y[:, split_index_1d .== 0]
	LOOCV_test_y = flatten_train_y[:, split_index_1d .== 1]
	#print(typeof(train_y[1]))
	#print(typeof(flatten_train_y[1]))
	# print(size(LOOCV_train_x,))
	split_index = zeros(Int, size(flatten_train_x, 2)-1)'
	split_index[end] = 1
	shuffle!(split_index)
	split_index_1d = vec(split_index)
	print(size(split_index_1d))
	k_values = 1:5
	accuracies = []
	#print(LOOCV_test_y[1])
	
	for k in k_values
           errors = 0
		   #for i in 1:size(LOOCV_train_x, 2)
			   
			   distances = [euclidean(vec(train_point) , vec(LOOCV_test_x)) for train_point in eachcol(LOOCV_train_x)]
			   sorted_indices = sortperm(distances)
			   k_nearest_labels = LOOCV_train_y[1, sorted_indices[1:k]]
			   predicted_label = mode(k_nearest_labels)
			   if predicted_label == LOOCV_test_y[1]
				   errors += 1
			   end
			   accuracy = errors / size(LOOCV_test_y, 2)
			   println(predicted_label)
			   println(LOOCV_test_y[1])
			   println(accuracy)
			   #push!(accuracies, accuracy)
		   #end
	end
				   
                 # test_point = LOOCV_test_x[:, i]
                 # test_label = LOOCV_test_y[1, i]

	 		
 #    #             distances = [norm(train_point - test_point) for train_point in
	# 			# eachcol(LOOCV_train_x)]
	# 		print(distances)
 #            
 #            println(sorted_indices)
	# 		println(sorted_indices[1:k])
 #            
	# 		println(k_nearest_labels)
 #            
	# 		println(predicted_label)
 ...
 #            end
            #end
			#println(typeof(predicted_label))
            #println(typeof(LOOCV_test_y))
        #     
        #     
        # end
	#print(accuracies)
	# for i in 1:size(LOOCV_train_x, 2)
	# 	distances = [euclidean(train_point .- LOOCV_test_x) for train_point in eachcol(LOOCV_train_x)]
	# for train_point in eachcol(LOOCV_train_x)
	# 	train_point_copy = copy(train_point) 
	# 	print(typeof(train_point_copy))
	# end
	#print(typeof(vec(LOOCV_test_x)))
	#print(typeof(convert(Vector{Float32}, LOOCV_test_x)))
...
begin
	...
	function knn_leave_one_out(train_x,train_y, k_values)
		#flatten_train_x = reshape(train_x, 784, :)
		flatten_train_x = reshape(train_x[:, :, 1:1000], 784, :)
		flatten_y = reshape(train_y, 1, :)
		flatten_train_y = flatten_y[:, 1:1000]
...
manifest_format = "2.0"
project_hash = "3a7b840e16aa078073ed7a18445d8b00221e6e34"
